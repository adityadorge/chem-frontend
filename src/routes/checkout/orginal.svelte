...
<main class="w-full px-5 md:px-8 lg:px-10 py-6" style="background-color: #f9fafb;">
  <!-- Progressive Milestone Bar OUTSIDE the Checkout container -->
  ...
  <div class="mx-auto w-[min(1680px,100vw-32px)]">
...
>
  {submitting ? "Submitting..." : "Submit Requisition"}
      ...
      <!-- Right: Order Summary -->
      <aside class="sticky top-8 h-fit min-h-[800px] rounded-xl bg-white p-6 shadow-md border border-gray-100 mt-6">
        <h2 class="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
          <ClipboardList class="h-5 w-5 text-indigo-600" /> Order Summary
        </h2>
        <div class="space-y-4 text-sm">
          <div>
            <span class="font-medium text-gray-700">Test:</span>
            <span class="ml-2">{testNameParam || "—"}</span>
          </div>
          <div>
            <span class="font-medium text-gray-700">Quantity:</span>
            <span class="ml-2">{testQtyParam ?? "—"}</span>
          </div>
          <div>
            <span class="font-medium text-gray-700">Customer:</span>
            <span class="ml-2">{sampleForm.customerName || "—"}</span>
          </div>
          <div>
            <span class="font-medium text-gray-700">Email:</span>
            <span class="ml-2">{sampleForm.email || "—"}</span>
          </div>
          <div>
            <span class="font-medium text-gray-700">Sample:</span>
            <span class="ml-2">{sampleForm.sampleNameOrBatch || "—"}</span>
          </div>
          <div>
            <span class="font-medium text-gray-700">Technique:</span>
            <span class="ml-2">{sampleForm.requestedTechnique || "—"}</span>
          </div>
          <div>
            <span class="font-medium text-gray-700">Pickup:</span>
            <span class="ml-2">{pickupConfirmed ? "Confirmed" : "Not set"}</span>
          </div>
        </div>
        <hr class="my-4" />
        ...
        <div class="flex flex-col gap-2">
          <button
            class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700"
